...
#include<stdlib.h>
#include<math.h>
// create a noise mono WAV file 1.0 sec long, samplerate=22050, bitspersample=4*8
main( ){
///////////////////////////////////////////////////////
...
// can't use anything like chunkid="RIFF";
sprintf(chunkid,"%c%c%c%c",'R','I','F','F');
for(k=0; k<4; k++){outstring[k]=chunkid[k];}
printf("chunkid[ ] = \"%s\"\n",outstring); // outstring has a string termination '\0' at the end
fwrite(chunkid,1,4,fpi); // read in RIFF
// compute chunksize
// 1 second of mono audiodata at 16-bits (2 bytes) per sample, samplerate = 22050
// = 22050*2*(1)=44,100 bytes of audiodata.
// The complete file will consist of a (44 byte WAV header +44,100)=44,144 bytes of audiodata
